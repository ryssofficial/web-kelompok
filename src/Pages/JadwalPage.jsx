...
// ─────────────────────────────────────────────
const StatBadge = ({ label, nilai, color }) => (
    <div style={{
...
const JadwalPage = ({ role = 'Guru' }) => {
    ...
    const [deleteConfirm, setDeleteConfirm] = useState(null); // id jadwal yang akan dihapus
    const [toast, setToast]             = useState(null);     // { msg, type }

    // ── Helpers ────────────────────────────────────────────────────────────
    const refresh = () => setJadwalData(jadwalManager.getAll());
    ...
    // ── Filter data berdasarkan tab hari aktif ─────────────────────────────
    const filteredData = activeHari === 'Semua'
        ? jadwalData
        : jadwalData.filter(j => j.hari === activeHari);

    // ── DataTable rows ─────────────────────────────────────────────────────
    const tableRows = filteredData.map((j, idx) => [
        idx + 1,
        j.hari,
        j.mapel,
        j.guru,
        ...
        j.ruangan,
        j.kelas,
        // Kolom Aksi — hanya tampil tombol untuk role Guru
        role === 'Guru' ? (
            <div key={j.id} style={{ display: 'flex', gap: '8px' }}>
                <StyledButton
                    label="✏️ Edit"
                    padding="6px 12px"
                    fontSize="12px"
    ...
    const handleSubmit = () => {
        try {
            if (editTarget) {
                // ── EDIT: rebuild dengan Builder lalu update ke Singleton ──
                ...
                // ── TAMBAH: build baru lalu tambah ke Singleton ──
    ...
    // ── Statistik ──────────────────────────────────────────────────────────
    const unikGuru   = [...new Set(jadwalData.map(j => j.guru))].length;
    const unikMapel  = [...new Set(jadwalData.map(j => j.mapel))].length;
    const unikKelas  = [...new Set(jadwalData.map(j => j.kelas))].length;
        ...
        <>
            ...
            >
                ...
                <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', marginBottom: '30px' }}>
                    ...
                    <StatBadge label="Mata Pelajaran" nilai={unikMapel}         color={HappyHuesTheme.tertiary} />
                    <StatBadge label="Guru / Pengajar" nilai={unikGuru}          color={HappyHuesTheme.secondary} />
                    <StatBadge label="Kelas"          nilai={unikKelas}          color='#3da9fc' />
                </div>
